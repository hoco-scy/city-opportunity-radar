...
const SOURCE_CONFIG = {
  ...
  "beijing-selection-program": {
    ...
    label: "北京市应届选调／优培公告（选调高校公开发布）",
    allowedDomains: ["career.buaa.edu.cn"],
    buaaSelection: {
      titlePattern: /北京市.*(?:定向)?选调.*(?:应届)?(?:优秀)?(?:大学|高校)毕业生.*(?:公告|招聘)/,
      issuerPattern: /(?:中共)?北京市委组织部|北京市人力资源和社会保障局/
    },
...
export async function collectBuaaSelectionProgram({ sourceId, fetchImpl = fetch, maxPages, maxNotices } = {}) {
  ...
  }, { collectionRoute: "北航就业网选调生公开栏目 API → 城市应届选调公告详情/附件" });
}
