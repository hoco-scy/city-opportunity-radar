...
async function main() {
  ...
  const [publicExamRun, buaa, iguopin, ncss, jqzp, picc, boe, crc] = await Promise.all([
    ...
    runMeasuredCollector("jqzp-beijing-soe", collectionFetch, (fetchImpl) => collectJqzpBeijingSoe({ city: recipes.city, fetchImpl }), performanceRecords).catch((error) => unavailableStructuredResult("jqzp-beijing-soe", error)),
  ...
  const structuredResults = new Map([jqzp, picc, boe, crc].map((result) => [result.sourceId, result]));
  ...
  let sourceChecks = await Promise.all(scheduledIds.map(async (sourceId) => {
    ...
    if (route.collector === "official-notice-feed") {
      const result = await runMeasuredCollector(sourceId, collectionFetch, (fetchImpl) => collectOfficialNoticeFeed({ source, fetchImpl }), performanceRecords);
  ...
  const officialNoticeMonitors = [...officialNoticeResults.entries()]
    ...
    .filter(Boolean);
  const structuredValues = [...structuredResults.values()];
  const structuredMetric = (field) => structuredValues.reduce((sum, result) => sum + (Number(result[field]) || 0), 0);
  const run = {
